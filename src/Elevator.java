...
public class Elevator implements ActionListener{
	...
	public static void startElevator() {
		...
		while (true) {
			if (currentFloor < floorSel) { //If the floor selection is above the current floor, invokes the goUp() method
				...
				soundPlayLoop(true, "src/ELEVATOR.wav");
				goUp();
				soundPlay(true, "src/DING.WAV");
				soundPlayLoop(false, "src/ELEVATOR.wav");
	...
	public static void soundPlayLoop(boolean play, String fileSource) {
		String source = new File(fileSource).toURI().toString();
		Media media = null;
		media = new Media(source);
		MediaPlayer mediaPlayerLoop = new MediaPlayer(media);
		mediaPlayerLoop.setCycleCount((Math.abs(currentFloor-floorSel)/6));
		if (play == false)
